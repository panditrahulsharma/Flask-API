       ...
       pathname = $(location).attr('pathname');
		pathname=pathname.split("/")
		 ...
		 if (pathname.length==3){
			...
			DataUrl="/DashboardData/"+pathname[2]
		}

		else if (pathname.length==4){

			DataUrl="/DashboardData/"+pathname[2]+"/"+pathname[3]
		}
   

		else {

			DataUrl="/DashboardData"
		}
...
// onload function
		$.ajax({
			url: DataUrl,//calling link when page load here
            ...
            $.each(response, function (key, data)
             {
				...
				car_data+='<td class="clout-done text-center"><span>'+data['status']+'</span></td>';
				car_data+='</tr>';
...
//// upload file==================================================================================================================
		    ...
		    $('#upload-file-btn1').click(function() {
				...
				car_data+='<td>'+"-"+'</td>';
				car_data+='<td id="'+random_id_status+'" class="clout-pending text-center"><span>'+"processing..."+'</span></td>';
				car_data+='</tr>';
				$('#car_tbody').append(car_data);
        				...
        				$.each(response, function (key, data)
				             {
									...
									document.getElementById(data['random_id_status']).innerHTML=data['status'];
										...
										document.getElementById(data['random_id_status']).classList.remove('clout-pending');
										document.getElementById(data['random_id_status']).classList.add('clout-done');

										

									// document.getElementById(data['random_id_status']).removeClass('clout-pending');
									// document.getElementById(data['random_id_status']).addClass('clout-done');
