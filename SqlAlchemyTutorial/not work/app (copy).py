...
class ContactAuthorDetails(db.Model):
    ...
    def create(self):
      ...
      db.session.commit()
      return self
    def __init__(self,Author,AuthorEmail,Telephone,KeyWords,FilePath):
...
class ContactAuthorDetailsSchema(ModelSchema):
    class Meta(ModelSchema.Meta):
        ...
        sqla_session = db.session
    id = fields.Number(dump_only=True)
    # ContactId = fields.Number(dump_only=True)
...
def paperPost():
    ...
    Authordata = {"Author":request.form['Author'],"AuthorEmail":request.form['AuthorEmail'],"Telephone":request.form['Telephone'],"KeyWords":request.form['KeyWords'],"FilePath":request.form['FilePath']}
